...
const createImage = async ( req, res) => {
    try {
        if (!req.file) return res.status(400).json({ message: "No se subio ninguna imagen."});
...
const deleteImage = async (req, res) => {
    try {
        ...
        if (!image) return res.status(404).json({ message: "Imagen no encontrada en este post" });
